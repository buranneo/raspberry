...
int enable_advertising(int advertising_interval, char *advertising_uuid, int major_number, int minor_number, int rssi_value, int dev_id)
{
  int device_id = hci_get_route(NULL);
  printf("device_id = %d\n", device_id);
  if (dev_id >= 0) device_id = dev_id;
  printf("set device_id = %d\n\n", device_id);
...
int disable_advertising(int dev_id)
{
  int device_id = hci_get_route(NULL);
  printf("device_id = %d\n\n", device_id);
  if (dev_id >= 0) device_id = dev_id;
...
void main(int argc, char **argv)
{
  if(argc != 7)
  {
    fprintf(stderr, "Usage: %s <advertisement time in ms> <UUID> <major number> <minor number> <RSSI calibration amount> <device id or -1>\n", argv[0]);
    exit(1);
  }

  int rc = enable_advertising(atoi(argv[1]), argv[2], atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), atoi(argv[6]));
  if(rc == 0)
  {
    ...
    disable_advertising(atoi(argv[6]));
  }
}
